...
//         i = 0
//         当 i < content 长度:
//             如果 content[i] 处是词内字符（判定看位置: 字母/数字/连字符无条件
//                 入词; '.' 与 '_' 仅当左右邻居均为字母/数字时入词 -- 三明治
//                 规则，唯一权威定义见 tokenizer.h 条款 2）:
//                 start = i
...
// - 词内字符判定是位置感知的: 判定 (content, i) 而非单字符 content[i]
//   （'.' 与 '_' 需查左右邻居），逻辑与 tokenizer 的 isWordCharAt 同构
